...
final signUpDataSourceProvider = Provider<EmailAuthDataSource>((ref) {
  return EmailAuthDataSourceImpl();
});

final signUpRepositoryProvider = Provider<EmailAuthRepository>((ref) {
  final data = ref.read(signUpDataSourceProvider);
  return EmailAuthRepositoryImpl(data);
});

final fetchAuthUsecaseProvider = Provider((ref) {
  final authRepo = ref.read(signUpRepositoryProvider);
